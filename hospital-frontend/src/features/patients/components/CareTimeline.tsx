    ...
    <div className="space-y-10 relative before:absolute before:left-5 before:top-2 before:bottom-2 before:w-0.5 before:bg-primary/10">
      {events.map((event, idx) => (
        <div key={idx} className="relative pl-14 group transition-all">
          ...
          {/* Timeline Icon Node */}
          <div className="absolute left-0 top-0 w-10 h-10 rounded-2xl bg-card border border-border/40 flex items-center justify-center text-primary shadow-lg group-hover:border-primary group-hover:shadow-primary/20 transition-all z-10">
          ...
          <div className="text-left space-y-2 font-sans antialiased">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-poppins font-black text-primary uppercase tracking-widest">
                {event.date}
              </span>
              <Badge variant="outline" className="text-[8px] font-poppins font-black uppercase border-primary/20 bg-primary/5">
                {event.type}
              </Badge>
            </div>

            <h4 className="text-lg font-poppins font-black text-foreground uppercase tracking-tight leading-none">
              {event.title}
            </h4>

            {/* Content Bubble */}
            <div className="p-5 bg-card/40 backdrop-blur-xl border border-border/40 rounded-3xl group-hover:border-primary/20 transition-all">
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-border/20">
                <div className="flex items-center gap-3">
                  ...
                  {/* SECURE DOCUMENT TAG */}
                  {event.hasReport && (
                    <Badge variant="outline" className="h-6 gap-2 text-[8px] font-poppins font-black uppercase text-primary bg-primary/5 hover:bg-primary/10 cursor-pointer transition-all">
