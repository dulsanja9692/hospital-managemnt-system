    ...
    <div className="flex flex-col min-h-full animate-in fade-in duration-700 text-left p-2 space-y-8 font-sans antialiased">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <Button 
          ...
          className="h-14 px-10 bg-primary text-white font-poppins font-black rounded-2xl shadow-xl shadow-primary/20 hover:scale-105 active:scale-95 transition-all gap-3"
        >
          <Plus size={20} />
          <span className="uppercase tracking-widest text-[10px]">Onboard New Specialist</span>
        </Button>
      </div>
      ...
      <ScrollArea className="h-[calc(100vh-350px)] pr-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-10">
          {filteredDoctors.length > 0 ? (
                ...
                <CardContent className="p-8">
                  <div className="flex items-start gap-5 text-left">
                    ...
                    <div className="pt-1 flex-1">
                      <h3 className="font-poppins font-black text-foreground text-xl tracking-tighter leading-tight group-hover:text-primary transition-colors uppercase">
                        {doc.name}
                      </h3>
                      <Badge variant="secondary" className="mt-2 bg-primary/5 text-primary border-primary/10 font-sans font-black uppercase text-[9px] tracking-widest px-3">
                        {doc.dept}
                      </Badge>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`text-[10px] font-poppins font-black uppercase tracking-widest ${doc.status === 'On Duty' ? 'text-green-600' : 'text-red-500/60'}`}>
                        {doc.status}
                      </span>
                    </div>
            ...
            <div className="col-span-full py-24 border-2 border-dashed border-border/40 rounded-[3.5rem] flex flex-col items-center justify-center text-muted-foreground/40 text-center">
              <Activity size={64} className="mb-6 opacity-10 animate-pulse" />
              <p className="font-poppins font-black uppercase tracking-[0.5em] text-xs">No Team Members Found</p>
            </div>
          )}
        </div>
      </ScrollArea>
      ...
      {/* 4. FOOTER TERMINAL */}
      <footer className="pt-8 border-t border-border/40 flex justify-between items-center opacity-30 font-sans">
        <p className="text-[9px] font-black tracking-[0.4em] uppercase">Mediflow Access • {new Date().getFullYear()}</p>
        <div className="flex items-center gap-6">
          <span className="text-[9px] font-black tracking-[0.4em] uppercase">Authorized Personnel Access</span>
          <Badge variant="outline" className="text-[8px] font-poppins font-black tracking-[0.4em] uppercase border-primary/20">V2.4.0_STABLE</Badge>
        </div>
      </footer>
